 ...
 * frames to execute
 */
void VelocityQueue::SetCommand(vector2d v, double va, bool kick, 
			       bool drib, int t)
{
  RVelocity vc;
...
void VelocityQueue::SetCommand(RVelocity vc, int t)
{
  ...
  if (t == tstamp[prev_cmd]) {
    set_cmd = prev_cmd;
    nr_commands--;
  } else if (t < tstamp[prev_cmd] || set_cmd == exec_cmd) return;
 ...
 * Get the next command from the queue
 */
void VelocityQueue::GetCommand(vector2d &v, double *va, 
			       bool *kick, bool *drib, int t)
{
  RVelocity vc;
